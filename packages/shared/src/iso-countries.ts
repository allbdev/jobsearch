/**
 * Every ISO 3166-1 alpha-2 code: the values a residence picker offers.
 *
 * Codes only. Names come from `Intl.DisplayNames` in the reader's language, so
 * there is no translated country list to maintain -- and the stored value is
 * what `job_eligibility.eligibleCountries` is matched against (#61).
 */
