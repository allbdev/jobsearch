...
| D15 | Auth: email + password, Google and GitHub, with opaque server-side sessions owned by the API | 🔶 in progress |
...
### D15 — Auth: password and OAuth, opaque sessions owned by the API

**What ships.** Email + password (argon2id), email verification, password reset,
and sign-in with Google and GitHub — what the Login design shows. All of it in
M3, split across PRs.

**Sessions are opaque and server-side, not JWTs.** A random 256-bit token in an
httpOnly cookie; the database keeps only its SHA-256. The reason is revocation:
signing out, resetting a password, and unlinking a compromised account must take
effect on the next request. A JWT cannot be withdrawn before it expires, and the
usual remedy — a denylist — is a sessions table with extra steps.

**The API owns auth; the web holds the cookie** (§7, D5). The browser only ever
talks to `apps/web`, so the cookie is set on the web's domain. On sign-in the API
returns the token in the response body, the web sets it as a cookie, and every
later API call carries it as `Authorization: Bearer`. The API sets no cookies and
knows nothing about the web's domain, which is what lets a second client exist
later. The web's middleware may redirect a request with no cookie; it never
decides whether a cookie is valid.

**OAuth callbacks land on the web**, which forwards the code to the API. State
and the PKCE verifier are generated by the API and held in a short-lived httpOnly
cookie between the redirect and the callback.

**Accounts are linked by verified email only.** Signing in with a provider whose
email matches an existing user links the two only if the provider asserts the
address is verified. When that existing account's own email was never verified,
linking also clears its password and ends its sessions: otherwise someone could
register a victim's address with a password first and keep access after the
victim signs in with Google (pre-hijacking).

**Not chosen: a hosted auth vendor** (Clerk, Auth0, Supabase Auth). It would be
faster to wire, but it puts user records outside our database, adds a per-user
bill to a product whose costs otherwise do not grow with users (COSTS.md §3), and
couples the web tier to a vendor SDK that D5 says should not hold logic.
